...
# print(f'The total price is ${BillPrice}.')


def GetPrice(product, quantity, price_dict):
    SubTotal = price_dict[product] * quantity
    print(f'{product}: {price_dict[product]} X {quantity} = ${SubTotal}')
    return SubTotal
...
for key, value in BuyingDict.items():
    BillPrice += GetPrice(key, value, PriceDict)
